...
node /^db[2-9]\.localdomain\.lan$/ {
  ...
  exec { 'mysql_change_master':
    command => "mysql -u replication_user --password=strong_replication_password -e \"CHANGE MASTER TO MASTER_HOST='db1.localdomain.lan', MASTER_USER='replication_user', MASTER_PASSWORD='strong_replication_password', MASTER_LOG_FILE='mysql-bin.000001', MASTER_LOG_POS=  107;\"",
    require => Class['::mysql::server'],
    unless  => "mysql -u replication_user --password=strong_replication_password -e 'SHOW SLAVE STATUS\\G' | grep 'Slave_IO_Running: Yes'",
  }

  exec { 'mysql_start_slave':
    command => "mysql -u replication_user --password=strong_replication_password -e 'START SLAVE;'",
